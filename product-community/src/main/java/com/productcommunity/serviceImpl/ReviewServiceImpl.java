...
public class ReviewServiceImpl implements ReviewService {
	...
	public List<Review> getReview(String productCode) {
		List<Review> reviewList = reviewRepo.findByProductCode(productCode);
		List<Review> approvedReviewList = new ArrayList<Review>();
		for (Review review : reviewList) {
			if (review.isApproved()) {
				approvedReviewList.add(review);
			}
		}
		return approvedReviewList;
	}
