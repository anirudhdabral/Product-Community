package com.productcommunity.controller;


import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.productcommunity.model.Stats;
import com.productcommunity.service.ProductService;
import com.productcommunity.service.ReviewService;
import com.productcommunity.service.UserService;

@RestController
@RequestMapping("/stats")
@CrossOrigin
public class StatsController {
	@Autowired
	private ProductService productService;
	
	@Autowired
	private UserService userService;
	
	@Autowired
	private ReviewService reviewService;
	
	@GetMapping("/getStats")
	public Stats getStats() {
		Stats stats = new Stats();
		int products =  productService.getProducts().size();
		int users = userService.getUsers().size();
		int reviews = reviewService.getReviews().size();
		
		stats.setNoOfProducts(products);
		stats.setNoOfUsers(users);
		stats.setNoOfReviews(reviews);
		
		return stats;
	}

}
